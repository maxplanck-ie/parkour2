...
describe("Ext.grid.plugin.BufferedRenderer", function () {
        ...
        function runTests(useBR) {
          ...
          describe("buffered rendering = " + useBR, function () {
            ...
            it(
              "should " +
                not +
                " create an instance on the ownerGrid for locking grids",
              function () {
            ...
            it(
              "should " +
                not +
                " create an instance by default on each child grid for locking grids",
              function () {
      ...
      describe("Load requests during scrolling", function () {
        ...
        beforeEach(function () {
          scrollToLoadBufferValue =
            Ext.grid.plugin.BufferedRenderer.prototype.scrollToLoadBuffer;
  ...
  describe("basic functionality with a buffered store", function () {
    ...
    it("should scroll to the bottom of a locking grid with no error", function () {
      ...
      lockedView.bufferedRenderer.scrollToLoadBuffer =
        normalView.bufferedRenderer.scrollToLoadBuffer = 0;
        ...
        function () {
          ...
          if (p) {
            ...
            if (
              ...
              lockedScroller.getPosition().y === normalScroller.getPosition().y
            ) {
  ...
  describe("gridpanel", function () {
    ...
    describe("locking grid with variableRowHeight", function () {
      itIE10p(
        "should keep the row heights on both sides synchronized",
        function () {
          var columns = [
              {
                text: "Col 1",
          ...
          waitsFor(
            function () {
              var reachedTargetRow =
                nodeCache.startIndex <= 99 && nodeCache.endIndex >= 99;

              // If row 99 is in the nodeCache, we're done
              if (reachedTargetRow) {
                return (
                  nodeCache.startIndex === lockedView.all.startIndex &&
                  lockedView.position === view.position &&
                  lockedView.bodyTop === view.bodyTop
                );
              } else {
                // Scroll incrementally until the correct end point is found
                grid.getScrollable().scrollBy(null, 40);
          ...
          waitsFor(
            function () {
              var reachedTargetRow =
                view.bufferedRenderer.getLastVisibleRowIndex() > 990;
          ...
          runs(function () {
            ...
            var mainHeight = view.el.down(".x-grid-item-container").getHeight(),
              partnerHeight = view.lockingPartner.el
                .down(".x-grid-item-container")
                .getHeight();
  ...
  describe("filtering the store", function () {
    ...
    itNotTouch(
      "should reset the cached position so the grid-item-container is at the top of the view on clearFilter",
      function () {
        var selModel;

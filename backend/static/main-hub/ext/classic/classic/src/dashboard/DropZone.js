...
Ext.define("Ext.dashboard.DropZone", {
  ...
  getOverEvent: function (dd, e, data) {
    ...
    for (i = 0; i < count; i += 2) {
      ...
      if (e.within(item.el)) {
        ...
        t = Math.min(80, w * 0.2);
        over.beforeAfter = t =
          over.extensible && (x < t ? -1 : x > w - t ? 1 : 0);
  ...
  notifyOver: function (dd, e, data) {
    ...
    if (
      (!hasListeners.validatedrop ||
        dashboard.fireEvent("validatedrop", over) !== false) &&
      (!hasListeners.beforedragover ||

...
Ext.define("Ext.grid.RowEditor", {
  ...
  reposition: function (animateConfig, fromScrollHandler) {
    ...
    if (row && Ext.isElement(row)) {
      ...
      if (animateConfig) {
        me.wrapAnim = wrapEl.addAnimation(
          Ext.applyIf(
            {
              to: {
                top: localY,
  ...
  getScrollDelta: function () {
    ...
    if (context) {
      ...
      if (deltaY < 0) {
        ...
        deltaY = Math.max(
          deltaY + editorHeight - clientHeight - body.getBorderWidth("b"),
          0,
        );
  ...
  calculateEditorTop: function (rowTop) {
    ...
    if (this._buttonsOnTop) {
      result -=
        this.body.dom.offsetHeight -
        this.context.row.offsetHeight -
  ...
  getClientWidth: function () {
    ...
    if (me.lockable) {
      lockedCmp =
        (grid.lockedGrid.collapsed && grid.lockedGrid.placeholder) ||
        grid.lockedGrid;
      result = lockedCmp
        .getRegion()
        .union(grid.scrollBody.el.getClientRegion()).width;
    } else {
  ...
  startEdit: function (record, columnHeader) {
    ...
    if (me.rendered && me.formAriaLabel) {
      label = Ext.String.formatEncode(
        me.formAriaLabel,
        me.formAriaLabelRowBase + context.rowIdx,
      );
  ...
  syncButtonPosition: function (context) {
    var me = this,
      ...
      scrollHeight = Math.max(
        0,
        me.scroller.getSize().y - me.scroller.getClientSize().y,
      ),
  ...
  doDestroy: function () {
    ...
    me.floatingButtons = me.tooltip = Ext.destroy(
      me.floatingButtons,
      me.tooltip,
    );

...
Ext.define("Ext.grid.plugin.BufferedRenderer", {
  ...
  init: function (grid) {
    ...
    me.scroller = view.lockingPartner
      ? view.ownerGrid.scrollable
      : view.getScrollable();
      ...
      if (renderedBlockHeight !== ownerContext.bodyHeight) {
        ...
        if (renderedBlockHeight < ownerContext.bodyHeight) {
          ...
          // Scroll up to bring it into view.
            ...
            me.bodyTop + renderedBlockHeight <
              me.scrollTop + me.viewClientHeight
          ) {
  ...
  onViewResize: function (view, width, height, oldWidth, oldHeight) {
    ...
    if (!oldHeight || height !== oldHeight) {
      ...
      me.viewClientHeight = view.lockingPartner
        ? (me.scroller && me.scroller.getClientSize().y) || height
              ...
              store.getRange(start, end, {
                callback: function (newRecords, start, end) {
                  ...
                  if (
                    lockingPartner &&
                    !fromLockingPartner &&
                    (ownerGrid.syncRowHeight ||
                      ownerGrid.syncRowHeightOnNextLayout)
                  ) {
                    lockingPartner.setViewSize(viewSize, true);
      ...
      if (rows.startIndex) {
        if (me.topOfViewCloseToEdge()) {
          ...
          if (requestStart > rows.startIndex) {
            requestStart =
              rows.startIndex + Math.floor(vscrollDistance / me.rowHeight);
          }
        }
      }
    }
  ...
  doRefreshView: function (range, startIndex, endIndex, options) {
    ...
    if (
      view.getVisibleColumnManager().getColumns().length &&
  ...
  onRangeFetched: function (range, start, end, options, fromLockingPartner) {
    ...
    if (start < rows.startIndex && end > rows.endIndex) {
      ...
      noOverlap =
        me.teleported || start > rows.endIndex || end < rows.startIndex;
      if (noOverlap) {
        view.clearViewEl(true);
        me.teleported = false;
      }
    ...
    if (
      me.variableRowHeight &&
      me.bodyHeight !== oldBodyHeight &&
      view.componentLayoutCounter === layoutCount
    ) {
      delete me.rowHeight;
      me.refreshSize();
    }
  ...
  getScrollHeight: function () {
    ...
    if (me.variableRowHeight) {
      ...
      // Calculate the scroll range based upon measured row height and our scrollPosition.
        ...
        scrollHeight =
          Math.floor((recCount - rowCount) * me.rowHeight) + me.bodyHeight;
    ...
    if (!me.destroyed) {
      me.store.getRange(start, end, {
        ...
        callback: function (range, start, end, options) {
          ...
          if (
            options.loadId === me.loadId &&
            me.scrollTop === loadScrollPosition
          ) {

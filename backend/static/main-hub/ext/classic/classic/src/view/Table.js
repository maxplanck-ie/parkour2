...
Ext.define("Ext.view.Table", {
  ...
  renderCell: function (
    ...
    cellValues.tdCls =
      ...
      cellValues.style =
        "";
    cellValues.unselectableAttr = enableTextSelection
      ? ""
      : 'unselectable="on"';
    ...
    cellValues.value =
      value == null || value.length === 0 ? column.emptyCellText : value;
    ...
    if (!me.cellFocused && me.all.getCount() && me.dataSource.getCount()) {
      ...
      // Focused a cell, or an interior element within a cell.
        ...
        // Testing whether the focusTarget isFocusable is a fix for IE. It can sometimes fire a focus event with the .x-scroll-scroller as the target
          ...
          focusTarget &&
          Ext.fly(focusTarget).isFocusable() &&
  ...
  findFocusPosition: function (from, currentPosition, forward, keyEvent) {
    ...
    focusTarget =
      tabbableChildren[
        Ext.Array.indexOf(tabbableChildren, from) + (forward ? 1 : -1)
      ];
    ...
    while (
      ...
      if (
        cell &&
        (tabbableChildren = Ext.fly(cell).findTabbableElements()).length
      ) {
        focusTarget =
  ...
  getDefaultFocusPosition: function (fromComponent) {
    var me = this,
      ...
      newPosition = new Ext.grid.CellContext(
        me.isNormalView &&
    ...
    if (fromComponent) {
      ...
      if (
        ...
        fromComponent.cellFocusable !== false &&
  ...
  onRemove: function (store, records, index) {
    ...
    if (
      me.actionPosition &&
      Ext.Array.indexOf(records, me.actionPosition.record) !== -1
    ) {
      me.actionPosition = null;
    }
  ...
  saveFocusState: function () {
    var me = this,
      ...
      focusPosition = actionableMode
        ? me.actionPosition
        : navModel.getPosition(true),
      activeElement = Ext.Element.getActiveElement(true),
      focusCell =
        focusPosition && focusPosition.view === me && focusPosition.getCell(),
      refocusRow,
      refocusCol;
  ...
  privates: {
    ...
    setActionableMode: function (enabled, position) {
      ...
      if (enabled) {
        ...
        if (
          me.el.contains(activeEl) &&
          !Ext.fly(activeEl).is(me.getCellSelector())
        ) {
          ...
          // See EXTJS-20395
          if (
            !(me.el.contains(activeEl) && activeEl.is(me.getCellSelector()))
          ) {
            position = null;
          }
        }
     ...
     * @private
     */
    findFirstActionableElement: function (
      keyEvent,
      focusRow,
      direction,
      forward,

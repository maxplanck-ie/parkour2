...
Ext.define("Ext.form.field.Tag", {
   ...
   */
  ariaHelpText:
    "Use Up and Down arrows to view available values, Enter to select. " +
  ...
  fieldSubTpl: [
    ...
    '<div id="{cmpId}-listWrapper" data-ref="listWrapper"' +
      (Ext.isGecko ? ' tabindex="-1"' : ""),
    '<tpl foreach="ariaElAttributes"> {$}="{.}"</tpl>',
    ' class="' +
      Ext.baseCSSPrefix +
      'tagfield {fieldCls} {typeCls} {typeCls}-{ui}"<tpl if="wrapperStyle"> style="{wrapperStyle}"</tpl>>',
    '<span id="{cmpId}-selectedText" data-ref="selectedText" aria-hidden="true" class="' +
      Ext.baseCSSPrefix +
      'hidden-clip"></span>',
    '<ul id="{cmpId}-itemList" data-ref="itemList" role="presentation" class="' +
      Ext.baseCSSPrefix +
      'tagfield-list{itemListCls}">',
    '<li id="{cmpId}-inputElCt" data-ref="inputElCt" role="presentation" class="' +
      Ext.baseCSSPrefix +
      'tagfield-input">',
    ...
    'class="' +
      Ext.baseCSSPrefix +
      'tagfield-input-field {inputElCls} {emptyCls}" autocomplete="off">',
    "</li>",
    "</ul>",
  ...
  initComponent: function () {
    ...
    if (!me.ariaHelp) {
      me.ariaHelp = me.createNewOnEnter
        ? me.ariaHelpTextEditable
        : me.ariaHelpText;
    }
    ...
    if (!me.ariaStaticRoles[me.ariaRole]) {
      ...
      if (attr) {
        ...
        attr["aria-describedby"] =
          id + "-selectedText " + (attr["aria-describedby"] || "");
      }
    }
  ...
  onKeyDown: function (e) {
    ...
    if (valueCount > 0 && rawValue === "") {
      ...
      if (
        (key === e.BACKSPACE && me.clearOnBackspace) ||
  ...
  getMultiSelectItemMarkup: function () {
    ...
    if (!me.multiSelectItemTpl) {
      ...
      me.multiSelectItemTpl = new Ext.XTemplate([
        '<tpl for=".">',
        '<li data-selectionIndex="{[xindex - 1]}" data-recordId="{internalId}" role="presentation" class="' +
          me.tagItemCls +
          childElCls,
  ...
  applyAriaSelectedText: function () {
    ...
    if (selectedText) {
      ...
      text = records.length
        ? me.getAriaSelectedText(records)
        : me.ariaNoneSelectedText;

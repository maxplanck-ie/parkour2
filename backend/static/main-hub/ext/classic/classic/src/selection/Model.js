...
Ext.define("Ext.selection.Model", {
  ...
  onNavigate: function (e) {
    ...
    var me = this,
      ...
      direction =
        key === keyEvent.UP ||
        key === keyEvent.PAGE_UP ||
        key === keyEvent.HOME ||
        (key === keyEvent.LEFT && changedRec)
          ? "up"
          : key === keyEvent.DOWN ||
              key === keyEvent.PAGE_DOWN ||
              key === keyEvent.END ||
              (key === keyEvent.RIGHT && changedRec)
            ? "down"
            : null;
  ...
  refresh: function () {
    ...
    if (
      !store ||
      !(selected.isCollection || selected.isBag || selected.isRows) ||
      !selected.getCount()
    ) {
      return;
    }

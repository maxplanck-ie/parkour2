  ...
  "Ext.selection.CheckboxModel",
  {
    ...
    beforeViewRender: function (view) {
      ...
      if (me.injectCheckbox !== false) {
        ...
        if (
          !ownerLockable ||
          (view.isLockedView && me.hasLockedHeader()) ||
    ...
    onColumnUnlock: function (lockable, column) {
      ...
      if (lockedColumns.length === 1 && lockedColumns[0] === me.column) {
        if (checkbox === "first") {
          ...
          checkbox =
            lockable.normalGrid.visibleColumnManager.getColumns().length;
        }
    ...
    onColumnLock: function (lockable, column) {
      ...
      if (lockedColumns.length === 1) {
        if (checkbox === "first") {
          ...
          checkbox =
            lockable.lockedGrid.visibleColumnManager.getColumns().length;
        }
    ...
    vetoSelection: function (e) {
      ...
      if (me.checkOnly) {
        isClick =
          e.type === column.triggerEvent &&
          e.getTarget(me.column.getCellSelector());

...
Ext.define("Ext.dataview.List", {
    ...
    for (i = 0; i < itemsCount; i++) {
      ...
      if (!item.$hidden && item.$position !== transY) {
        ...
        if (
          ...
          header &&
          me.headerIndices &&
          me.headerIndices[item.$dataIndex]
        ) {
  ...
  onContainerResize: function (container, size) {
    ...
    if (!me.headerHeight) {
      ...
      me.headerHeight =
        parseInt(pinnedHeaderElement.getHeight(), 10) -
  ...
  updatePinHeaders: function (pinnedHeaders) {
    ...
    if (me.isPainted()) {
      ...
      me.translate(
        pinnedHeader,
        0,
        pinnedHeaders ? pinnedHeader.$position : -10000,
      );
    }
  },
  ...
  onItemTrigger: function (me, index, target, record, e) {
    if (
      !(
        ...
        e.getTarget(me.disclosureSelector)
      )
    ) {
  ...
  updateRowLines: function (rowLines) {
    this.container.innerElement.toggleCls(
      this.noRowLinesCls,
      rowLines === false,
    );
  },
  ...
  privates: {
    ...
    handleIndexBarSize: function () {
      ...
      if (!task) {
        me.indexBarTask = task = new Ext.util.DelayedTask(
          me.doHandleIndexBarSize,
          me,
        );
      }
      task.delay(1);

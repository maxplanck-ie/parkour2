Ext.define("Ext.ux.DateRangePicker", {
        ...
        buttons: [
          ...
          },
          {
            ...
            handler: function (btn) {
              ...
              datePicker.hideMenu();
              if (
                btn.prev("[drpItemRole=drpPresetPeriodsBtn]").hasVisibleMenu()
              ) {
  ...
  initComponent: function () {
    ...
    me.drpDefaults = me.drpDefaults
      ? Ext.apply(defaults, me.drpDefaults)
      : defaults;
    ...
    me.on({
      afterrender: function () {
        ...
        if (
          drpConfig.selectedEnd &&
          Ext.Date.parse(drpConfig.selectedEnd, "Y-m-d", true)
        ) {
  ...
  setRange: function () {
    ...
    me.setText(
      drpConfig.mainBtnTextPrefix +
        '<b style="color: ' +
        drpConfig.mainBtnTextColor +
        '">' +
        dFrom +
        " - " +
        dTo +
    ...
    if (
      drpConfig.bindDateFields &&
      drpConfig.boundStartField &&
      drpConfig.boundEndField &&
      drpConfig.boundStartField.isComponent &&
      drpConfig.boundStartField.isXType("datefield") &&
      drpConfig.boundEndField.isComponent &&
  ...
  setSecondMinDate: function () {
    ...
    if (drpConfig.showTimePickers) {
      ...
      if (dStart.getTime() === dEnd.getTime()) {
        ...
        var tFrom = panel
          .down("timefield[drpItemRole=timePickerFrom]")
          .getValue();
  ...
  getPickerValue: function () {
    ...
    var periodInfo = {
      periodYmdHis:
        Ext.Date.format(retObj.startDateObj, "Y-m-d H:i:s") +
        " - " +
        Ext.Date.format(retObj.endDateObj, "Y-m-d H:i:s"),
      periodFmt:
        Ext.Date.format(
          retObj.startDateObj,

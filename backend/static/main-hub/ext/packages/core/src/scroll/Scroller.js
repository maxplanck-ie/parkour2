  ...
  "Ext.scroll.Scroller",
  {
    ...
    privates: {
      ...
      updateSpacerXY: function (pos) {
        ...
        if (shortfall > 0) {
          sStyle.marginTop =
            Math.min(shortfall, me.maxSpacerMargin || 0) + "px";
        }
      },
      ...
      doIsInView: function (el, skipCheck) {
        ...
        if (el && (skipCheck || myEl.contains(el) || (c && c.owns(el)))) {
          ...
          result.x =
            elRegion.right > myElRegion.left &&
            elRegion.left < myElRegion.right;
          result.y =
            elRegion.bottom > myElRegion.top &&
            elRegion.top < myElRegion.bottom;
        }
        return result;
      },
          ...
          if (interval) {
            ...
            if (x) {
              style["-ms-scroll-snap-points-x"] =
                "snapInterval(0px, " + x + "px)";
            }

            if (y) {
              style["-ms-scroll-snap-points-y"] =
                "snapInterval(0px, " + y + "px)";
            }
          }
        }
      },
  ...
  function (Scroller) {
    ...
    Ext.setViewportScroller = function (scroller) {
      if (Scroller.viewport !== scroller) {
        ...
        Scroller.viewport = scroller.isScroller
          ? scroller

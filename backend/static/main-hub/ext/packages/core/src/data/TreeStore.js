...
Ext.define("Ext.data.TreeStore", {
  ...
  beforeNodeRemove: function (parentNode, childNodes, isMove, removeRange) {
    ...
    if (startNode) {
      ...
      removeRange[1] =
        me.indexOfNextVisibleNode(childNodes[childNodes.length - 1]) -
        removeRange[0];
    } else {
      removeRange[0] = -1;
      removeRange[1] = 0;
  ...
  onNodeSort: function (node, childNodes) {
    ...
    if (
      (me.indexOf(node) !== -1 && node.isExpanded()) ||
      (node === me.getRoot() && !me.getRootVisible())
    ) {
      Ext.suspendLayouts();
  ...
  privates: {
    ...
    filterNodes: function (node, filterFn, parentVisible) {
      var me = this,
        ...
        match =
          (filterFn(node) && parentVisible) ||
          (node.isRoot() && !me.getRootVisible()),
        childNodes = node.childNodes,
        len = childNodes && childNodes.length,
        i,
        matchingChildren;

  ...
  function () {
      ...
      addAnchorOffset = function (target, anchorSize, relativePosition) {
        ...
        if (relativePosition != null && anchorSize) {
          adjustParams[0] =
            ...
            adjustParams[3] =
              0;
          adjustParams[relativePosition] =
            anchorSize.y * magnitude[relativePosition];
        ...
        if (relativePosition != null) {
          ...
          if (relativePosition & 1) {
            ...
            if (target.getHeight() > minOverlap) {
              ...
              if (
                overlapLength < target.width &&
                overlapLength < anchorSize.x + 4
              ) {
                if (overlapLength < minOverlap) {
                  if (
                    overlapLine.getAnchorPoint_c()[1] >
                    target.getAnchorPoint_c()[1]
                  ) {
          ...
          // The result is above or below the target.
            ...
            if (target.getWidth() > minOverlap) {
              ...
              if (
                overlapLength < target.height &&
                overlapLength < anchorSize.x + 4
              ) {
                if (overlapLength < minOverlap) {
                  if (
                    overlapLine.getAnchorPoint_c()[0] >
                    target.getAnchorPoint_c()[0]
                  ) {
    ...
    return {
          ...
          if (wasConstrained && !overlap) {
            ...
            // If edge aligning, we must completely exclude the region
              ...
              if (result.intersect(targetPlusAnchorOffset)) {
                ...
                options.minWidth &&
                result.getWidth() > inside.getWidth()
              ) {
                result.setPosition(0, result.y);
                ...
                options.minHeight &&
                result.getHeight() > inside.getHeight()
              ) {
                result.setPosition(result.x, 0);
                result.setHeight(
        ...
        if (inside) {
          for (i = 0; i < translations.length; i++) {
            ...
            if (minHeight) {
              ...
              if (inside.contains(testRegion)) {
                if (
                  !sizeConstrainedSolution ||
                  testRegion.getArea() >
                    sizeConstrainedSolution.region.getArea()
                ) {
                  sizeConstrainedSolution = {
                    region: testRegion,
                    align: t[2],
            ...
            if (intersection) {
              ...
              if (
                !leastBadSolution ||
                (intersection && leastBadSolution.area < intersection)
              ) {
                leastBadSolution = {
                  region: testRegion,
                  align: t[2],
          ...
          if (!result) {
            ...
            if (sizeConstrainedSolution) {
              ...
              other.constrainWidth =
                sizeConstrainedSolution.region.constrainWidth;
              other.constrainHeight =
                sizeConstrainedSolution.region.constrainHeight;
            }
      ...
      getAnchorPoint: function (align, rtl) {
        align =
          typeof align === "string"
            ? this.getAlignInfo(align + "-tl", rtl)
            : align;

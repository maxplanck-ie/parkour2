...
Ext.Base = (function (flexSetter) {
  ...
  Base.addMembers({
    ...
    $reap: function () {
      ...
      for (prop in me) {
        if (
          (!protectedProps || !protectedProps[prop]) &&
          me.hasOwnProperty(prop)
        ) {
          value = me[prop];
          type = typeof value;
          ...
          if (
            type === "object" ||
            (type === "function" && !value.$noClearOnDestroy)
          ) {
            me[prop] = null;
          }
        }
      ...
      if (
        me.clearPrototypeOnDestroy &&
        !me.$vetoClearingPrototypeOnDestroy &&

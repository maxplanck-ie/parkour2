...
Ext.define("Ext.chart.series.sprite.Pie3DPart", {
  ...
  alphaUpdater: function (attr) {
    ...
    if (
      (opacity !== oldOpacity && (opacity === 1 || oldOpacity === 1)) ||
      (fillOpacity !== oldFillOpacity &&
        (fillOpacity === 1 || oldFillOpacity === 1))
    ) {
  ...
  render: function (surface, ctx) {
    ...
    if (
      !attr.globalAlpha ||
      Ext.Number.isEqual(attr.startAngle, attr.endAngle, 1e-8)
    ) {
      return;
    }
  ...
  sideRenderer: function (path, position) {
    var attr = this.attr,
      ...
      isFullPie =
        !attr.startAngle &&
        Ext.Number.isEqual(Math.PI * 2, attr.endAngle, 0.0000001),

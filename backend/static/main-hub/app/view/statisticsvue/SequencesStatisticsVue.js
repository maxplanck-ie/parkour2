Ext.define("MainHub.view.statisticsvue.SequencesStatisticsVue", {
  extend: "Ext.container.Container",
  xtype: "sequences-statistics-vue",

  layout: "fit",
  iframeDomId: "sequencesStatisticsIframe",
  ...
  addIframe: function () {
    this.add({
      ...
      html:
        ...
        '/vue/sequences_statistics" width="100%" height="100%" frameborder="0"></iframe>'
    });
  },

  reloadIframe: function () {
    var iframe = document.getElementById(this.iframeDomId);
    if (iframe) iframe.contentWindow.location.href = iframe.src;
  },

  removeIframe: function () {
    var iframeComponent = this.down("#" + this.iframeDomId);
    if (iframeComponent) this.remove(iframeComponent, true);
  },

  setupParentClickRelay: function () {
    if (this.parentPointerListener) return;
    var me = this;
  ...
  relayPointerEventToIframe: function (event) {
    ...
    var iframe = document.getElementById(this.iframeDomId);
    if (!iframe || event.target === iframe || !iframe.contentWindow) return;
    iframe.contentWindow.postMessage(
      { source: "mainhub-ext", type: "parent-pointer-down" },
      window.location.origin
    );
  },

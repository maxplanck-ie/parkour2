...
- Libraries and Samples: every column search box (Status, Name, Type, Plate Coord, Barcode, Pool Paths, GMO, Date, Input Type, Comment, Organism, Protocol, Analysis Type, Coord, Index I7/I5, Length, Flowcell IDs, Sequencers, plus the existing Index Type/I7 ID/I5 ID) now filters server-side across the full result set, not just the loaded page, with gray hint text/tooltips showing the expected syntax. Plate Coord filters against the server-computed value from #339, matched by re-deriving it for every request name still passing the other active filters. The redundant Status/Protocol/Analysis Type/Sequencer/Read Length dropdowns were removed from Advanced Filters, which now only holds Changed Ownership (no column of its own). The server-side filter debounce was bumped from 500ms to 800ms to avoid reloading mid-typing.

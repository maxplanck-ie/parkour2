...
- Libraries & Samples: every column search box now filters server-side across the full result set, not just the loaded page, with gray hint text and tooltips showing the expected syntax. Plate Coord filters against the server-computed value from #339. The Status/Protocol/Analysis Type/Sequencer/Read Length dropdowns are back in Advanced Filters too, now as shortcuts onto the same column search boxes rather than a separate ID-based filter path. The server-side filter debounce was bumped from 500ms to 800ms to avoid reloading mid-typing.

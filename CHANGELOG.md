...
- Fixed `frontend/package.json`'s `start-dev` script binding Vite to port 5174 while `nginx-server.conf`, `frontend.Dockerfile`, and `misc/Caddyfile` all expect 5173 — the two had drifted apart repeatedly (see `83e76f4c`); `start-dev` now uses 5173 like everything else, so dev deployments don't 502.

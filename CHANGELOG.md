...
- Added header-filter search boxes to nearly every remaining column across Incoming, Preparation, Load FCs, Pooling, Run Statistics, and Sequences Statistics: numeric/QC/cost columns get Invoicing's comparison syntax (`>100`, `50-200`, etc. — now shared via `numericHeaderFilter.js` instead of duplicated per module), plain-text columns (Protocol, Comment, Index IDs, Coord, etc.) get a substring filter, and the two categorical columns (Unit facility, GMO facility) get a dropdown (`headerFilter: "select"`) instead of free text.

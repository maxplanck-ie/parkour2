...
- Added a `make dev-fix` target: deploys like `make dev`, but also loads demo fixture data. `make dev-ez` now loads demo fixture data instead of an empty DB. (PR #354.)- Adopted VueUse composables and Motion across the frontend (browser listeners, storage, debounce, clipboard, resize, and dialog animations), including animated save-confirmation dialogs in the Request Editor. (PRs #350, #352.)

...
- Demo fixture data no longer names real MPI-IE staff or the institute itself: Principal Investigators are now historical genomics/bioinformatics pioneers (Dayhoff, Jeffreys, Kanehisa, Moreno-Estrada, Soodyall, Charpentier, Graves) spanning six continents, each attributed to the real institution where they made their key contribution, and covering a wider spread of analysis types (including Hi-C and small RNA-Seq). The `Bioinfo`/`DeepSeq` facility PIs used by Duties keep matching sentinel names so the Duties "Responsible/Backup Person" pickers stay populated. (PR #354.)
- `make dev-ez` now loads demo fixture data (like `make dev-fix`) instead of an empty DB. (PR #354.)

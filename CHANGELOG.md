...
- Added two box plots to the Usage page, "Turnaround Time by Analysis Type" and "Turnaround Time by PI", showing the spread (min/median/max) of days from request approval to sequencing start. The Usage page charts are now laid out in two rows (Libraries & Samples, Organizations, Principal Investigators on top, with Principal Investigators given half the row's width; Analysis Types, Turnaround by Analysis Type, Turnaround by PI below in equal thirds). Also truncated long x-axis labels (over 18 characters) on all Usage charts, kept Libraries & Samples/Organizations labels horizontal, and unified all Usage charts onto one shared color palette — non-Libraries/Samples charts now pick up where the Libraries/Samples blue/orange pair leaves off, instead of restarting at the same two colors or using a clashing separate palette. (PR #354.)
- Renamed the "Stats" nav dropdown's "Primary"/"Secondary" entries to "Per Lane"/"Per S/L", naming each by the data it actually shows (sequencer-lane QC vs. per-sample/library QC). (PR #354.)
- Added a `make dev-fix` target: deploys like `make dev`, but also loads demo fixture data — including a handful of processed requests with approval/sequencing timestamps set, so the new Usage turnaround-time charts have something to show out of the box. (PR #354.)
- Demo fixture data no longer names real MPI-IE staff or the institute itself: Principal Investigators and Organizations are now fictional, and cover a wider spread of analysis types (including Hi-C and small RNA-Seq) across more PIs. (PR #354.)
...
- Added a staff API endpoint for looking up PIs, so dissectBCL no longer needs a static configuration list. (PR #317, direct commit `5ddb4fe0`.)
- Fixed bulk create/update endpoints breaking after a dependency upgrade. (Direct commit `3190d000`.)
- New Vue-based Invoicing page, replacing the old ExtJS grid. (PR #308.)
- Infrastructure: moved the backend base image to Debian 12 for Playwright/Firefox support. (PR #319.)
- RO-Crate export: improved reliability on per-record errors, richer metadata, and an offline-browsable HTML preview in the export. (PRs #314, #315, #316, #327.)

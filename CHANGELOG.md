...
- Libraries and Samples: every column search box (Status, Name, Type, Plate Coord, Barcode, Pool Paths, GMO, Date, Input Type, Comment, Organism, Protocol, Analysis Type, Coord, Index I7/I5, Length, Flowcell IDs, Sequencers, plus the existing Index Type/I7 ID/I5 ID) now filters server-side across the full result set, not just the loaded page, with gray hint text/tooltips showing the expected syntax. Plate Coord filters against the server-computed value from #339, matched by re-deriving it for every request name still passing the other active filters. The Status/Protocol/Analysis Type/Sequencer/Read Length dropdowns are back in Advanced Filters too, now as shortcuts onto the same column search boxes (picking a dropdown value fills in and stays in sync with the matching column's box) rather than a separate ID-based filter path. The server-side filter debounce was bumped from 500ms to 800ms to avoid reloading mid-typing.
...
- Added header-filter search boxes to nearly every remaining column across Incoming, Preparation, Load FCs, Pooling, Run Statistics, and Sequences Statistics: numeric/QC/cost columns get Invoicing's comparison syntax (`>100`, `50-200`, etc. — now shared via `numericHeaderFilter.js` instead of duplicated per module), plain-text columns (Protocol, Comment, Index IDs, Coord, etc.) get a substring filter via a new shared `textFilterConfig()` helper (`textHeaderFilter.js`) instead of the same 2-3 line block repeated ~60 times, and GMO facility (Incoming) gets a text filter that also parses yes/no like L&S's GMO filter does.
- Audited every header filter added above against the actual database: Date columns (Preparation, Load FCs, Pooling, Invoicing) now show a `DD.MM.YYYY` hint instead of generic "Filter...", and every numeric filter's example values were checked against real data ranges and corrected where the shared generic hint (`>100  50-200`) didn't fit the column (e.g. RQN is 0-11 not 0-100, loading concentration averages ~4pM not >100, off-species reads average well under 1%). Added a tooltip to Invoicing's "%" column explaining what it means: this request's share of the pool's total sequencing depth, times the pool's lane count on the flowcell (it's a composite value, not a plain percentage, so it's kept as a text filter).
...
- CI: fixed the weekly dependency-update workflow failing outright since zizmor 1.20 started requiring SHA pins on official (not just third-party) actions; `zizmor --fix=all` now actually applies the SHA-pinning fix. zizmor's own version is now pinned (bumped weekly with a PR note, like the other tool pins) instead of floating, so a future policy change shows up as a reviewable diff instead of a silent CI failure. Added `.github/zizmor.yml` to disable the handful of pre-existing, non-auto-fixable findings (unrelated to action pinning) that would otherwise block this job forever.

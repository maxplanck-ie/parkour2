...
- Demo fixture data no longer names real MPI-IE staff or the institute itself: Principal Investigators and Organizations are now fictional, each PI assigned to the fictional organization matching their name's regional origin, and covering a wider spread of analysis types (including Hi-C and small RNA-Seq) across more PIs. The `Bioinfo`/`DeepSeq` facility PIs used by Duties keep matching sentinel names so the Duties "Responsible/Backup Person" pickers stay populated. (PR #354.)

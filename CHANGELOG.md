...
- Duties: migrated the legacy AG Grid to the shared Vue/Tabulator table, retaining inline editing, date-period filtering, and per-column filters; added Playwright coverage for add/search/period-filter/inline-edit. Follow-up cleanup made the page consistent with the rest of the app: the Add Duty form now uses `v-model` instead of manual `document.getElementById` DOM manipulation, the API client uses the shared `createAxiosObject()` instead of a standalone `axios.create()`, and the search bar reuses the same row-matches-search pattern as Run/Sequences Statistics instead of a bespoke per-field filter. Reworked the header to match the rest of the app (title left, search/period-filter/Add Duty button right, fixing the title's rightward misalignment along the way) and turned the always-visible Add Duty side panel into a modal dialog opened from the header button, with the same focus-trap/Escape/click-outside behavior as other dialogs in the app. Default period filter is now Past 1 Year (was Ongoing); Start/End Date columns are now sortable, and the table sorts by End Date descending (most recent first) by default (was Start Date ascending).
...
- Libraries & Samples filtering: every column search box now filters server-side across the full result set (not just the loaded page), with gray hint text/tooltips and an 800ms debounce (up from 500ms). Advanced Filters gained I7/I5 Index range filters (From/To, e.g. N701 to N729) and got its Status/Protocol/Analysis Type/Sequencer/Read Length dropdowns back, now as shortcuts onto the same column search boxes rather than a separate ID-based filter path. "Plate Coord" (A1..H12) now computes server-side (`CompleteLibraryData`/`CompleteSampleData.plate_coord`), ranking libraries and samples together by barcode within each request name — fixed a bug (found via a production-snapshot check) where a barcode-only key mishandled two 2019 Sample records sharing a legacy barcode; it now keys on request name, record type, and primary key. Also fixed global search returning no results for an Index Type that exists in the data (the query never included `index_type_name`).
- Incoming, Preparation, Load FCs, Pooling, Run Statistics, and Sequences Statistics: added gray "Filter..." hint text to existing column search boxes (matching Invoicing's pattern) and header-filter search boxes to nearly every remaining column — numeric/QC/cost columns get a shared comparison-syntax filter (`>100`, `50-200`, etc., via `numericHeaderFilter.js`) with example values checked against real per-column data ranges, plain-text columns get a shared substring filter (`textHeaderFilter.js`), date columns get a `DD.MM.YYYY` hint, and GMO facility (Incoming) parses yes/no like L&S's GMO filter. Also added a header tooltip explaining Invoicing's composite "%" column.
- Frontend shell: removed the legacy ExtJS shell in favor of a native Vue app shell/nav (migrated Usage charts to Vue/ECharts, dropped the dead iframe-bridge code, deleted `backend/static/main-hub` and its leftover Makefile/template references); restyled the top nav bar (teal brand + beige bar, pill buttons, shortened/renamed labels, Usage moved under the Stats dropdown, dropdown-clipping fix, two-row user actions, restored Stats chart-line icon with microscope/magnifying-glass for Primary/Secondary, icon-only collapse when the bar runs out of room); fixed Vite dev server HMR/WebSocket proxying and unified dev/prod onto the same port; added a `/` redirect to the default Vue page.
- Emails: fixed the notification header logo (an outdated "S"-swirl mark at a distorted 17x26px, replaced with the current DNA-helix mark at 24x24); transactional emails now have a consistent header/logo layout, personalized approval emails, a password-change confirmation email, and a styled error page for expired approval links.
- Fixed Load Flowcells "Apply to All" not persisting loading concentration/PhiX values.
- CI: fixed the weekly dependency-update workflow failing outright since zizmor 1.20 started requiring SHA pins on official (not just third-party) actions; `zizmor --fix=all` now actually applies the SHA-pinning fix, and zizmor's own version is now pinned (bumped weekly with a PR note) instead of floating. Added `.github/zizmor.yml` to disable the handful of pre-existing, non-auto-fixable findings (unrelated to action pinning) that would otherwise block this job forever.
...
- Infrastructure: moved the backend base image to Debian 12 for Playwright/Firefox support.
- RO-Crate export: per-record error isolation, richer Person/Organization/ISA metadata, fastq stub entities for dissectBCL, and an offline-browsable HTML preview in the zip.

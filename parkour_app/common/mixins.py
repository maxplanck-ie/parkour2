...
class MultiEditMixin:
    ...
    def edit(self, request):
        ...
        serializer = self.get_serializer(data=post_data, instance=objects, many=True)
    ...
    def _update_valid(self, data):
        ...
        objects = self._get_model().objects.filter(pk__in=ids)
        serializer = self.get_serializer(data=data, instance=objects, many=True)
        serializer.is_valid()
        serializer.save()
...
class LibrarySampleMultiEditMixin:
    """
    ...
    def edit(self, request):
        ...
        libraries_ok, libraries_no_invalid = self._update_objects(
            self.library_model,
            self.library_serializer,
            library_ids,
            library_post_data,
        )

        samples_ok, samples_no_invalid = self._update_objects(
            self.sample_model, self.sample_serializer, sample_ids, sample_post_data
        )

        result = [libraries_ok, libraries_no_invalid, samples_ok, samples_no_invalid]
    ...
    def _update_objects(self, model_class, serializer_class, ids, data):
        ...
        if serializer.is_valid():
            ...
            if any(valid_data):
                ...
                self._update_valid(model_class, serializer_class, new_ids, valid_data)
            else:
                objects_ok = False
            no_invalid = False
    ...
    def _update_valid(self, model_class, serializer_class, ids, valid_data):
        """Update valid objects."""
        objects = model_class.objects.filter(pk__in=ids)
        serializer = serializer_class(data=valid_data, instance=objects, many=True)
        serializer.is_valid()
        serializer.save()
